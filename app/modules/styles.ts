...
export default defineNuxtModule((_, nuxt) => {
    ...
    addTemplate({
        ...
        getContents: () => {
            console.log('[Nuxt Module] Generating colors.scss!');
            return scss;
        },
        write: true,
    });
    ...
    addTemplate({
        filename: 'colors.ts',
        getContents: () => {
            console.log('[Nuxt Module] Generating colors.ts!');
            return `export const pageColors = ${ JSON.stringify(variables) };\nexport const pageThemes = ${ JSON.stringify(themes) };`;
        },
        write: true,
    });
    ...
    nuxt.options.alias['#colors'] = resolve(nuxt.options.buildDir, 'colors.ts');
    addImports([
        {
            name: 'pageColors',
            from: '#colors',
